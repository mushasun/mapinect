...
namespace mapinect {
	...
	vector<ofxVec3f> HandDetector::unifyHandVertex(PointCloud<PointXYZ>::Ptr handHull)
		{
			//cout << "pre unify: " << handHull->size() << endl;
			vector<vector<ofxVec3f>> tmp;
			vector<ofxVec3f> final;
			//pcl::io::savePCDFileASCII ("preunify.pcd", *handHull);

			for (int i = 0; i < handHull->size(); i++) {
				ofxVec3f pto = POINTXYZ_OFXVEC3F(handHull->at(i));
				bool unified = false;
	...
	vector<ofxVec3f> HandDetector::getFingers()
	{
		//aplano mano - debug
		for(int i = 0; i < hand->size(); i ++)
			hand->at(i).z = 0;
	...
	int HandDetector::findCloserFingerTo(ofxVec3f currentFinger,vector<ofxVec3f> unifiedHull,int handDirection)
	{
		int idx = -1;
	...
	float HandDetector::checkFingers(vector<ofxVec3f> fingers)
	{
		...
		ofxVec3f vCentroid = ofxVec3f(handCentroid.x,handCentroid.y, handCentroid.z);
		ofxVec3f thumbTip = fingers.at(minDistIdx);
		...
		{
			int fingersFound = 1;
			ofxVec3f currentFinger = vCentroid - thumbTip;
			ofxVec3f currentFingerTip = thumbTip;
			...
			while(fingerFounded)
			{
				...
				if(fingerIdx != -1)
				{
					...
					ofxVec3f newFinger = vCentroid - currentFingerTip;
	...
	float HandDetector::IsHand()
	{
		if(!onTable(hand, table))
		{
			...
			if(hand->size() > 0)
			{
				...
				//Busco los dedos
				vector<ofxVec3f> fingers = getFingers();
