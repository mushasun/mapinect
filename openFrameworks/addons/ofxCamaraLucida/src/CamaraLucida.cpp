...
namespace cml 
{
								...
								string xml_config_filename)
	{
		...
		calib.near_ = xml_config.getValue("near", 0.1);
		calib.far_ = xml_config.getValue("far", 20.0);
	...
	void CamaraLucida::load_data(string kinect_calibration_filename, string proj_calibration_filename)
	{
		...
		convertKKopencv2opengl(rgb_int, calib.rgb_width, calib.rgb_height, calib.near_, calib.far_, rgb_KK);
		...
		convertKKopencv2opengl(depth_int, calib.depth_width, calib.depth_height, calib.near_, calib.far_, depth_KK);
		...
		convertKKopencv2opengl(proj_int, calib.proj_width, calib.proj_height, calib.near_, calib.far_, proj_KK);
	...
	void CamaraLucida::convertKKopencv2opengl(CvMat* opencvKK, float width, float height, float near, float far, float* openglKK)
	{
		...
		float E = - (calib.far_ + calib.near_) / (calib.far_ - calib.near_);
		float F = -2. * calib.far_ * calib.near_ / (calib.far_ - calib.near_);
